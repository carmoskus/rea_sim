...
num.reads = 2.5e6
a = Reduce('+', replicate(num.tech, sapply(1:conf$ns.g, function (i) rmultinom(1, num.reads*sizes[i], c(meansA[,i], sum(meansA[,i])*0.4))[1:length(means),]), simplify=FALSE))

## Group b is altered
b = Reduce('+', replicate(num.tech, sapply(conf$ns.g + 1:conf$ns.g, function (i) rmultinom(1, num.reads*sizes[i], c(meansB[,i-conf$ns.g], sum(meansB[,i-conf$ns.g])*0.4))[1:length(means),]), simplify=FALSE))
