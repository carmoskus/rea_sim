...
name = paste0("wilcox_log_", arg.norm)
...
x = lapply(arg.start:arg.end, function (arg.num) {
    ...
    ## Do t-tests
    test = function (row) {
        r = wilcox.test(row ~ col.info$group)
        c(mean=mean(row), log2FC=median(row), t=r$statistic, p.value=r$p.value)
    }
    res = t(apply(counts, 1, test))
